...
//
// Base chance to break the Nth tackle attempt of the play (0-indexed): 1st 55%, 2nd 40%, 3rd 10%,
...
// On a break the carrier loses momentum (speed halved) and must re-accelerate to top speed.
...
// keep running (at halved speed), and stay live. Interception returns can't break tackles.
//
...
export function runTackleDetection(state, io, dt, rng = Math.random) {
  ...
  for (const d of tacklers.values()) {
    ...
    if (!isReturn) {
      ...
      const chance = tackleBreakChance(ratingOf(carrier, 'runPower'), broken)
